...
import { Cloud, Globe, Server, Database, ShieldCheck, Zap } from 'lucide-react';
...
export default function TopologyMap({ state }: { state: EnvironmentState }) {
    ...
    // In recovering state, packets shouldn't flow past the LB
    const instancePacketVariants = {
        active: {
            offsetDistance: ["0%", "100%"],
            transition: {
                duration: 1.5,
                repeat: Infinity,
                ease: "linear",
                delay: 0.75, // Staggered from first leg
                repeatDelay: 0.5
            }
        },
        recovering: {
    ...
    return (
        <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6 h-[400px] relative overflow-hidden flex items-center justify-center">
            ...
            <div className="relative w-full max-w-4xl flex justify-between items-center z-10 px-12">
                ...
                {/* LOAD BALANCER NODE */}
                <div className="flex flex-col items-center gap-2">
                    <div className="w-16 h-16 rounded-lg bg-blue-900/20 border-2 border-blue-500/50 flex items-center justify-center text-blue-400">
                        <ShieldCheck size={28} />
                    </div>
                    <span className="text-xs text-blue-400 font-mono">LB-REGION-1</span>
                </div>
                ...
                <div className="relative">
                    <motion.div
                        ...
                        className="w-20 h-20 rounded-xl border-2 flex items-center justify-center text-white shadow-lg transition-colors duration-300"
                    >
                    ...
                    <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 whitespace-nowrap flex flex-col items-center">
                        <span className={`text-xs font-bold font-mono ${isRecovering ? 'text-red-500' : 'text-purple-400'}`}>
                ...
                {/* DB NODE */}
                <div className="flex flex-col items-center gap-2">
                    <div className="w-16 h-16 rounded-full bg-emerald-900/20 border-2 border-emerald-500/50 flex items-center justify-center text-emerald-400">
                        <Database size={24} />
                    </div>
                    <span className="text-xs text-emerald-400 font-mono">PRIMARY-DB</span>
                </div>
